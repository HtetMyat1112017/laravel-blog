                    ...
                    <div class="card-body">
                        <form action="{{route('post.update',$post->id)}}" method="post">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="category" class="form-select @error('category') is-invalid @enderror" id="">
                                    ...
                                    @foreach(\App\Models\Category::all() as $categories)
                                        <option value="{{$categories->id}}" {{ $categories->id == old('category',$post->category_id) ? 'selected' : '' }}>{{$categories->name}}</option>
                                    @endforeach
                                </select>
                                @error('category')
                                <small class="text-danger">{{$message}}</small>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="description" id="" cols="30" rows="10" class="form-control @error('description')  is-invalid @enderror" name="description">{{$post->description}}</textarea>
                                @error('description')
                                <small class="text-danger">{{$message}}</small>
                                @enderror
                            </div>
